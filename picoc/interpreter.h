...
struct FuncDef
{
	...
	void (*Intrinsic)(struct ParseState *, struct Value *, struct Value **,
			int); /* intrinsic call address or NULL */
...
{
	void (*Func)(struct ParseState *Parser, struct Value *, struct Value **,
			int);
...
enum ParseResult
{
	ParseResultEOF, ParseResultError, ParseResultOk
};
...
struct Picoc_IO_Config_Struct
{
	void (*printf)(Picoc * pc, const char * message, ...);
	void (*dprintf)(Picoc * pc, const char * message, ...);
	void (*printVf)(Picoc * pc, const char * message, va_list);
	char * (*readline)(char *Buf, int MaxLen, const char *Prompt);
	char * (*tempName)(Picoc * pc, char * tempNameBuffer);
	int (*getch)(void);
};

struct Picoc_Callbacks_Struct
{
	void (*clean)(Picoc * pc);
	void (*init)(Picoc * pc);
	void (*exit)(Picoc * pc, int retval);
};

struct Picoc_User_Library_Struct
{
	const char * filename;
	void (*include_init)(Picoc * pc);
	void (*clean)(Picoc * pc);
	const struct LibraryFunction * table;
};

struct Picoc_User_Config_Struct
{
	//memory setup
	unsigned char * heapBase;
	unsigned long heapSize;
	//printing functions
	const struct Picoc_IO_Config_Struct * io;
	const struct Picoc_Callbacks_Struct * cb;
	const struct Picoc_User_Library_Struct * lib_table;
};

typedef struct Picoc_User_Config_Struct Picoc_User_Config;
typedef struct Picoc_Callbacks_Struct Picoc_Callbacks;
typedef struct Picoc_IO_Config_Struct Picoc_IO;
typedef struct Picoc_User_Library_Struct Picoc_User_Library_Table;
...
struct Picoc_Struct
{
	...
	const char * name;
	const struct Picoc_User_Config_Struct * platform;
...
void LexInitParser(struct ParseState *Parser, Picoc *pc, const char *SourceText,
		void *TokenSource, char *FileName, int RunIt, int SetDebugMode);
...
void *LexCopyTokens(struct ParseState *StartParser,
...
/* heap.c */
void HeapInit(Picoc *pc, int StackSize, unsigned char * heapBase);
...
struct Value *VariableAllocValueFromType(Picoc *pc, struct ParseState *Parser,
		struct ValueType *Typ, int IsLValue, struct Value *LValueFrom,
		int OnHeap);
...
void VariableDefinePlatformVar(Picoc *pc, struct ParseState *Parser,
		char *Ident, struct ValueType *Typ, union AnyValue *FromValue,
		int IsWritable);
...
void LibraryAdd(Picoc *pc, struct Table *GlobalTable, const char *LibraryName,
		const struct LibraryFunction *FuncList);
...
void IncludeRegister(Picoc *pc, const char *IncludeName,
		void (*SetupFunction)(Picoc *pc),const struct LibraryFunction *FuncList,
		const char *SetupCSource);

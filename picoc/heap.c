...
void HeapInit(Picoc *pc, int StackOrHeapSize, unsigned char * heapBase)
{
	int Count;
	int AlignOffset = 0;

	pc->HeapSize = StackOrHeapSize - 4;
	pc->HeapBottom = heapBase + pc->HeapSize; /* the bottom of the (downward-growing) heap */
	pc->StackFrame = heapBase; /* the current stack frame */
	pc->HeapStackTop = heapBase; /* the top of the stack */
	pc->HeapMemory = pc->StackFrame;

	while (((unsigned long) &pc->HeapMemory[AlignOffset]
	...
	*(void **) (pc->StackFrame) = NULL;
	pc->HeapBottom = &(pc->HeapMemory)[StackOrHeapSize - sizeof(ALIGN_TYPE)
			+ AlignOffset];
	pc->FreeListBig = NULL;
...
void *HeapAllocMem(Picoc *pc, int Size)
{
	...
	size_t AllocSize = MEM_ALIGN(Size) + MEM_ALIGN(sizeof(NewMem->Size));
	int Bucket;
	void *ReturnMem;
	...
	if (Bucket < FREELIST_BUCKETS && pc->FreeListBucket[Bucket] != NULL)
	{
		...
		assert(
				(unsigned long )NewMem >= (unsigned long )&(pc->HeapMemory)[0]
				&& (unsigned char * )NewMem - &(pc->HeapMemory)[0]
				< pc->HeapSize);
		pc->FreeListBucket[Bucket] = *(struct AllocNode **) NewMem;
		assert(
				pc->FreeListBucket[Bucket] == NULL || ((unsigned long)pc->FreeListBucket[Bucket] >= (unsigned long)&(pc->HeapMemory)[0] && (unsigned char *)pc->FreeListBucket[Bucket] - &(pc->HeapMemory)[0] < pc->HeapSize));
		NewMem->Size = AllocSize;
	}
	else if (pc->FreeListBig != NULL)
	{
		/* grab the first item from the "big" freelist we can fit in */
		for (FreeNode = &pc->FreeListBig;
				*FreeNode != NULL && (*FreeNode)->Size < AllocSize; FreeNode =
						&(*FreeNode)->NextFree)
		{
		}
		...
		if (*FreeNode != NULL)
		{
			assert(
					...
					>= (unsigned long )&(pc->HeapMemory)[0]
					&& (unsigned char * )*FreeNode
					- &(pc->HeapMemory)[0] < pc->HeapSize); assert((*FreeNode)->Size < pc->HeapSize && (*FreeNode)->Size > 0);
...
#endif
				NewMem = (void *) ((char *) *FreeNode + (*FreeNode)->Size
						- AllocSize);
				assert(
						(unsigned long )NewMem
						>= (unsigned long )&(pc->HeapMemory)[0]
...
void HeapFreeMem(Picoc *pc, void *Mem)
{
	...
	free(Mem);
#else
	struct AllocNode *MemNode = (struct AllocNode *) ((char *) Mem
			- MEM_ALIGN(sizeof(MemNode->Size)));
...
#endif
	assert(
			(unsigned long )Mem >= (unsigned long )&(pc->HeapMemory)[0]
			&& (unsigned char * )Mem - &(pc->HeapMemory)[0]
			< pc->HeapSize); assert(MemNode->Size < pc->HeapSize && MemNode->Size > 0);
	if (Mem == NULL)
		return;
...
#endif
		assert(
				pc->FreeListBig == NULL || ((unsigned long)pc->FreeListBig >= (unsigned long)&(pc->HeapMemory)[0] && (unsigned char *)pc->FreeListBig - &(pc->HeapMemory)[0] < pc->HeapSize));
